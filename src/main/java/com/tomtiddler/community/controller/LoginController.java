...
public class LoginController implements CommunityConst {
    ...
    public void getKaptcha(
        ...
        Cookie cookie = new Cookie("kaptchaOwner", kapchaOwner);
        cookie.setMaxAge(60);
    ...
    public String login(String username, String password, String code, boolean rememberMe, Model model,
        ...
        if (StringUtils.isBlank(kaptcha) || StringUtils.isBlank(code) || !kaptcha.equalsIgnoreCase(code)) {
            System.out.println("当前session中验证码为：" + kaptcha);
            System.out.println("当前传入的验证码为：" + code);
            model.addAttribute("codeMsg", "验证码不正确！");
            return "/site/login";
        }

        //检查账号，密码
        int expiredSeconds =  rememberMe ? REMEMBER_EXPIRED_SECONDS : DEFAULT_EXPIRED_SEONDS;
